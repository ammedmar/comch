...
from clesto.simplicial import SimplicialEZ_element, SimplicialEZ
...
class TestSurjection_element(unittest.TestCase):
    ...
    def test_call_simplicial(self):
        ...
        x = SimplicialEZ.standard_element(3)
        ds_x = s.boundary()(x)
        d_sx = s(x).boundary()
        s_dx = s(x.boundary())
        self.assertEqual(d_sx - ((-1)**(s.degree)) * s_dx, ds_x)

        x = SimplicialEZ_element({((0, 1, 2), (3, 4), (5, 6)): 1})
        ds_x = s.boundary()(x, 2)
        d_sx = s(x, 2).boundary()
        s_dx = s(x.boundary(), 2)
        self.assertEqual(d_sx - ((-1)**(s.degree)) * s_dx, ds_x)
    ...
    def test_call_cubical(self):
        s = self.x
        y = CubicalEZ.standard_element(3)
        ds_y = s.boundary()(y)
        d_sy = s(y).boundary()
        sdy = s(y.boundary())
