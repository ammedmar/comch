...
class Surjection_element(Module_element):
    ...
    def orbit(self, representation='trivial', return_permutation=False):
        ...
        for k, v in self.items():
            ...
            print(self, type(self.create({k: v})))
            print(permutation * self.create({k: v}))
    ...
    def _pcompose(self, other, k):
        ...
        '''
        answer = self.zero()
        for (k1, coeff1), (k2, coeff2) in product(self.items(), other.items()):
...
s = Surjection_element({(2, 1, 2, 1): 1}, convention='McClure-Smith')
